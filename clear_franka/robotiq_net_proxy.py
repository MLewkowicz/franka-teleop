...
Server side (run on the robot host where the gripper is physically reachable):
    ...
    python -m clear_franka.robotiq_net_proxy serve --com-port /dev/ttyUSB0

Or copy just this file to the robot and run:

    python robotiq_net_proxy.py serve --com-port /dev/ttyUSB0
...
def _main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Robotiq gripper ZMQ server/client")
